...
function getMongoURI(dbname) {
    ...
    if (MONGO_USER && MONGO_PWD && MONGO_CLUSTER) {
        console.log('Connecting to MongoDB at', `mongodb+srv://${MONGO_USER}:*****@${MONGO_CLUSTER}/${dbname}`);
...
export function connect(dbname) {
    mongoose
        ...
        if (error.message.includes('IP')) {
            ...
            console.error('   Please add your current IP address to the Atlas whitelist.');
